	...
	if (xhr.readyState == 4) {
		if (xhr.status == 200) {
			...
			for (let i=0; i<3; i++) {
				...
				m_items[currentitem.id] = new MenuItem(currentitem.href, currentitem.title, currentitem.id);
			}
			for (let i=3; i<6; i++) {
				currentitem = items.menu_items[i];
				sub_m_items[currentitem.id] = new MenuItem(currentitem.href, currentitem.title, currentitem.id);
			}
			let menu = new Menu("my_menu", "menu_class", m_items);			
			let sub_menu = new SubMenu("my_sub_menu", "sub_menu_class", {0: sub_m_items["item_4"], 1: [sub_m_items["item_5"], sub_m_items["item_6"]]});
